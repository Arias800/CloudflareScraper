...
A simple Python module to bypass Cloudflare's anti-bot page (also known as "I'm Under Attack Mode", or IUAM), implemented with [Requests](https://github.com/kennethreitz/requests).
...
* The version number from `pip show CloudflareScraper`.
...
scraper = CloudflareScraper.create_scraper()  # returns a CloudflareScraper instance
# Or: scraper = CloudflareScraper.CloudflareScraper()  # CloudflareScraper inherits from requests.Session
...
session.headers = ...
scraper = CloudflareScraper.create_scraper(sess=session)
```

Unfortunately, not all of Requests' session attributes are easily transferable, so if you run into problems with this, you should replace your initial `sess = requests.session()` call with `sess = CloudflareScraper.create_scraper()`.
...
```python
scraper = CloudflareScraper.create_scraper(delay=10)
```
...
To retrieve just the cookies (as a dictionary), use `CloudflareScraper.get_tokens()`. To retrieve them as a full `Cookie` HTTP header, use `CloudflareScraper.get_cookie_string()`.
...
If your tool already has a particular user-agent configured, you can make cloudflare-scrape use it with `CloudflareScraper.get_tokens("http://somesite.com/", user_agent="User-Agent Here")` (also works for `get_cookie_string`). Otherwise, a randomly selected user-agent will be used.
...
tokens, user_agent = CloudflareScraper.get_tokens("http://somesite.com", proxies=proxies)
print tokens
...
cookie_value, user_agent = CloudflareScraper.get_cookie_string("http://somesite.com")
...
# tokens, user_agent = CloudflareScraper.get_tokens("http://somesite.com")
...
cookie_arg, user_agent = CloudflareScraper.get_cookie_string("http://somesite.com")

# With a custom user-agent string you can optionally provide:

# ua = "Scraping Bot"
# cookie_arg, user_agent = CloudflareScraper.get_cookie_string("http://somesite.com", user_agent=ua)
...
cookie_arg, user_agent = CloudflareScraper.get_cookie_string(url)
